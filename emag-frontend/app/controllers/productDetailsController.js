...
app.controller("productDetailsController", function ($scope, $location, $routeParams, $http, sessionService) {

    var productId = $routeParams.id;
    var userId = sessionService.getSession();
    $scope.comment = {"productId": productId, "userId": userId, value: "", stars: ""};
    $scope.addProductComment = function () {
        $http({
            url: "http://localhost:7377/comment" + "/addComment",
            method: "POST",
            params: {
                "productId": productId,
                "userId": userId,
                "value": $scope.comment.value,
